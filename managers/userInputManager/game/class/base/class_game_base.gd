...
func general_processing_pipeline(tce_event_uuid : String, value) -> void: 
	if self._is_tce_signaling_uuid_matching(tce_event_uuid, ["game", "*"]): # Safety to ensure that only valid requests are processed
		...
		if UserInputManager.is_current_gui_context_menu():
			...
			# UserInputManager.TCE_INPUT_EVENTS_TO_TCE_EVENT_UUID_LUT
			var _tmp_eventsMapping : Array = [
			...
			# {"LUT":["movement", "channel1"], "UI_ACTION": ""},
			# 	{"LUT":["movement", "channel2"], "UI_ACTION": ""},
			# 	{"LUT":["movement", "channel3"], "UI_ACTION": ""},

			for _event in _tmp_eventsMapping:
				_tmp_keyChain = ["*"]
			...
			var _tmp_movementChannelMapping : Array = [
				{"LUT":["movement", "channel1"]},
				{"LUT":["movement", "channel2"]},
				{"LUT":["movement", "channel3"]}
			]

			for _event in _tmp_movementChannelMapping:
				_tmp_keyChain = ["*"]
				_tmp_keyChain += DictionaryParsing.get_dict_element_via_keychain(UserInputManager.TCE_INPUT_EVENTS_TO_TCE_EVENT_UUID_LUT, _event["LUT"])
				if UserInputManager.match_tce_event_uuid(tce_event_uuid, _tmp_keyChain):
					if value is Vector2:
						...
						if abs(_upDown) >= 0.501:
							if _upDown < 0:
								UserInputManager.trigger_fake_input_event("ui_down", true)

							elif _upDown > 0:
								UserInputManager.trigger_fake_input_event("ui_up", true)

						if abs(_leftRight) >= 0.501:
							if _leftRight < 0:
								UserInputManager.trigger_fake_input_event("ui_left", true)

							elif _leftRight > 0:
								UserInputManager.trigger_fake_input_event("ui_right", true)
